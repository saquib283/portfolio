...
const Hero = ({ settings }: { settings?: any }) => {
    ...
    return (
        <section id="home" className="min-h-[100vh] flex items-center justify-center relative pt-16 overflow-hidden">
            <ThreeScene />
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10 w-full text-center">
                ...
                >
                    <div className="mb-6 md:mb-8 relative w-20 h-20 md:w-32 md:h-32 mx-auto rounded-full overflow-hidden border-4 border-surface shadow-2xl">
                        <Image
                            src="/profile.jpeg"
                            alt="Profile"
                            fill
                    ...
                    <h1 className="font-display font-bold text-3xl sm:text-5xl md:text-8xl tracking-tighter text-primary mb-4 md:mb-6 leading-[1.1]" dangerouslySetInnerHTML={{ __html: title.split('\n').join('<br/>') }} />
                </motion.div>

                <motion.p
                    className="text-secondary text-sm md:text-xl max-w-2xl mx-auto font-light leading-relaxed px-2"
                    initial={{ opacity: 0, y: 30 }}
                    animate={{ opacity: 1, y: 0 }}
                ...
                <motion.div
                    className="flex flex-col sm:flex-row gap-2.5 sm:gap-3 md:gap-4 justify-center items-center pt-6 md:pt-8 w-full max-w-sm sm:max-w-none mx-auto"
                    initial={{ opacity: 0, y: 30 }}
                    animate={{ opacity: 1, y: 0 }}
